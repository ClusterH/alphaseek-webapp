...
const TeamItem: React.FC<{ item: ITeamItem; isCoreTeam?: boolean }> = ({ item, isCoreTeam }) => {
  return (
    <ItemWrapper padding={'120px 45px'} borderRadius={themeBorderRadius.small}>
      <AvatarWrapper src={item.avatar} width={isCoreTeam ? '50%' : '40%'} />
      {item.role && (
        <TextWrapper color={'text2'} fontFamily={'title'} fontSize={'sm'} fontWeight={'bold'} lineHeight={20} letterSpacing={'--0.02em'}>
          {item.role}
        </TextWrapper>
      )}
      <GradientTextWrapper
